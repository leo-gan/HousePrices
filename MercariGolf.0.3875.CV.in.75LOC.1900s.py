...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    # concat 'name' and 'brand_name', take care of NA
    df['name'] = df['name'].fillna('') + ' ' + df['brand_name'].fillna('')
    # concat 'item_description', 'name', and 'category_name', take care of NA
    df['text'] = (df['item_description'].fillna('') + ' ' + df['name'] + ' ' + df['category_name'].fillna(''))
    # do not store the original 'name', 'brand_name', 'item_description', 'category_name'
    # store only 2 new fields and 'shipping', 'item_condition_id'
